...
readonly FLUXIONPromptDefault="$CRed[${CSBlu}$CSYel@$CSWht$HOSTNAME$CClr$CRed]-[$CSYel~$CClr$CRed]$CClr "
...
startup() {
  ...
  local requiredCLITools=(
    ...
    "fuser:psmisc" "killall:psmisc" "figlet"
  )
    ...
    if [ $FLUXIONMissingDependencies -eq 1 ]  && [ $FLUXIONSkipDependencies -eq 1 ];then
        echo -e "\n\n"
        format_center_literals "[ ${CSRed}Missing dependencies: try to install using ./main.sh -i${CClr} ]"
...
fluxion_handle_target_change() {
  ...
  if ! load_attack "$EwTPath/attacks/$FluxionAttack/attack.conf"; then
...
fluxion_set_language() {
  ...
  source "$EwTPath/language/$FluxionLanguage.sh"
...
if ! xterm -title "$FLUXIONScannerHeader" $TOPLEFTBIG \
  ...
  # Syntheize scan WindowRation results from output file "dump-01.csv."
...
fluxion_get_target() {
  ...
  for candidateAPInfo in "${FluxionTargetCandidates[@]}"; do
    ...
    if [ $power -eq -1 ]; then
      ...
      candidatesQuality[i]="??"
    elif [ $power -le $NoiseFloor ]; then
      candidatesQuality[i]=0
    elif [ $power -gt $NoiseCeiling ]; then
      ...
      candidatesQuality[i]=$(( \
        (${candidatesPower[i]} * 10 - $NoiseFloor * 10) / \
        (($NoiseCeiling - $NoiseFloor) / 10) \
      ))
    fi
  done
...
fluxion_hash_set_path() {
  ...
  while [ ! "$FluxionHashPath" ]; do
    ...
    echo
    echo -e "$FLUXIONVLine $EwTPathToHandshakeFileQuery"
    echo -e "$FLUXIONVLine $EwTPathToHandshakeFileReturnTip"
    echo
...
fluxion_set_attack() {
  ...
  local attacks
  readarray -t attacks < <(ls -1 "$EwTPath/attacks")

  local descriptions
  readarray -t descriptions < <(
    head -n 3 "$EwTPath/attacks/"*"/language/$FluxionLanguage.sh" | \
  ...
  for attack in "${attacks[@]}"; do
    local identifier=$(
      head -n 3 "$EwTPath/attacks/$attack/language/$FluxionLanguage.sh" | \
...
fluxion_prep_attack() {
  local -r path="$EwTPath/attacks/$FluxionAttack"
