...
CaptivePortalPassLog="$EwTPath/attacks/Captive Portal/pwdlog"
CaptivePortalNetLog="$EwTPath/attacks/Captive Portal/netlog"
...
captive_portal_set_authenticator() {
  ...
  case "$CaptivePortalAuthenticatorMode" in
    "hash"*)
      ...
      if [ ! "$CaptivePortalHashPath" ]; then
        CaptivePortalHashPath="$EwTPath/attacks/Handshake Snooper/handshakes/$FluxionTargetSSIDClean-$FluxionTargetMAC.cap"
      fi
...
captive_portal_set_certificate() {
  ...
  if [ \
    -f "$EwTPath/attacks/Captive Portal/certificate/server.pem" -a \
    -s "$EwTPath/attacks/Captive Portal/certificate/server.pem" \
    ]; then
    cp "$EwTPath/attacks/Captive Portal/certificate/server.pem" \
...
captive_portal_set_user_interface() {
  local -r attackPath="$EwTPath/attacks/Captive Portal"
  ...
  case "$site" in
    ...
    #  source "$EwTPath/attacks/Captive Portal/generic/languages/$siteLanguage.lang"
...
captive_portal_set_attack() {
  ...
  for authenticatorFile in "${authenticatorFiles[@]}"; do
    cp "$EwTPath/attacks/Captive Portal/lib/$authenticatorFile" \
  ...
  cp -r "$EwTPath/attacks/Captive Portal/lib/connectivity responses/" \
...
captive_portal_generic() {
  ...
  base64 -d "$EwTPath/attacks/Captive Portal/generic/assets" >"$FLUXIONWorkspacePath/file.zip"
...
if [ $FLUXIONEnable5GHZ -eq 1 ];then
    cp -r "$EwTPath/attacks/Captive Portal/deauth-ng.py" "$FLUXIONWorkspacePath/captive_portal/deauth-ng.py"
